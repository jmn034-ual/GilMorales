...
public class Lista_denuncias_finalizadas extends Lista_denuncias {
	...
	public void cargarDenunciasFinalizadas() {
		...
		for(int i = 0; i < denuncias.size(); i++) {
			if(tamanio >= 3) {					
				this.getVaadinVerticalLayout().as(VerticalLayout.class).add(new HorizontalLayout(new Lista_denuncias_finalizadas_item(denuncias.get(i), this),
						new Lista_denuncias_finalizadas_item(denuncias.get(++i), this), new Lista_denuncias_finalizadas_item(denuncias.get(i+=1), this)));
				tamanio -= 3;
			}else if(tamanio == 2){
				HorizontalLayout horizontal2 = new HorizontalLayout(new Lista_denuncias_finalizadas_item(denuncias.get(i), this),
						new Lista_denuncias_finalizadas_item(denuncias.get(++i), this));
				horizontal2.getStyle().set("width", "66.66%");

				this.getVaadinVerticalLayout().as(VerticalLayout.class).add(horizontal2);
			}else {
				HorizontalLayout horizontal = new HorizontalLayout(new Lista_denuncias_finalizadas_item(denuncias.get(i), this));
