package interfaz;

public class Ver_publicacion_propia__Comercial_ {
	private Video _video;
	private ImageIcon _usuarioIcono;
	private button _verPerfilB;
	private button _eliminarB;
	private Label _geolocalizacionL;
	private TextArea _descripcionTA;
	private Label _fechaL;
	private int _numeroMeGustas;
	private int _numeroComentarios;
	private Label _meGustasL;
	private Label _comentarioL;
	private button _verMeGustasB;
	private button _verComentariosB;
	private int _numeroVisualizaciones;
	private Label _visualizacionesL;
	public Lista_publicaciones__Comercial__item _lista_publicaciones__Comercial_;
	public Ver_me_gustas_publicaci�n__Comercial_ _ver_me_gustas_publicaci�n__Comercial_;
	public Ver_comentarios__Comercial_ _ver_comentarios__Comercial_;
	public lista_Top_comentarios__Publicaci�n_comercial_ _lista_Top_comentarios__Publicaci�n_comercial_;

	public void Ver_me_gustas_publicaci�n__Comercial_() {
		throw new UnsupportedOperationException();
	}

	public void Ver_comentarios__Comercial_() {
		throw new UnsupportedOperationException();
	}

	public void lista_Top_comentarios__Publicaci�n_comercial_() {
		throw new UnsupportedOperationException();
	}

	public void Eliminar() {
		throw new UnsupportedOperationException();
	}

	public void Visualizaciones() {
		throw new UnsupportedOperationException();
	}
}