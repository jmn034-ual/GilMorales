...
public class Lista_comentarios_Usuario_registrado_item extends Lista_Comentarios_Vista_Usuario_No_registrado_item {
	...
	public Lista_comentarios_Usuario_registrado_item(Lista_comentarios_Usuario_registrado interfaz,
		...
		this.user = this.bd.cargarUsuarioRegistrado(
				this._lista_comentarios__Usuario_registrado_._ver_comentarios__Usuario_Registrado_.user.getID());
	...
	public void Dar_me_gusta_comentario() {
		...
		if (this.comentario.getEsComentadoPor().getID() != this.user.getID()) {
			this.getMeGustaB().addClickListener(event -> {
				...
				this.getNumeroMeGusta().setText(this.comentario.getNumeroMeGustas() + "");
				this.publiacion = this.bd.cargarPublicacion(
						this._lista_comentarios__Usuario_registrado_._ver_comentarios__Usuario_Registrado_.publicacion
								.getIdPublicacion());
				this.user = this.bd.cargarUsuarioRegistrado(
						this._lista_comentarios__Usuario_registrado_._ver_comentarios__Usuario_Registrado_.user
								.getID());
				if (gusta)
					this.getMeGustaB().setIcon(new Icon(VaadinIcon.HEART));
				else
