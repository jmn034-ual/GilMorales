...
public class Ver_comentarios extends VistaVerComentarios {
		...
		this.getBotonNombreUsuario().addClickListener(event -> {
			...
			if (interfaz instanceof Ver_comentarios_Usuario_Registrado) {
				...
				if (auxAdmin.publicacion.getPerteneceA() != null)
					this.verPerfilAdmin = new Ver_perfil_Administrador(cabeceraTopAux,
							auxAdmin.publicacion.getPerteneceA());
				else
					this.verPerfilAdmin = new Ver_perfil_Administrador(cabeceraTopAux,
							auxAdmin.publicacion.getEsPublicada());
	...
	public void NumeroSeguidores() {
		if (this.publicacion.getPerteneceA() != null) {
			UsuarioRegistrado user = this.bd.cargarUsuarioRegistrado(this.publicacion.getPerteneceA().getID());
			if (user.seguidor != null)
